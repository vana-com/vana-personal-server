...
from .grant_models import (
    GrantFile, GrantValidationOptions, GrantValidationResult,
    GrantValidationError, GrantExpiredError, GranteeMismatchError,
    OperationNotAllowedError, GrantSchemaError
)
...
def validate_grant_expiry(grant_file: GrantFile, current_time: Optional[int] = None) -> None:
    """
...
def validate_grant(
    data: Any,
    options: Optional[GrantValidationOptions] = None
    ...
    if errors:
        if options.throw_on_error:
            ...
            if "error" in first_error and isinstance(first_error["error"], Exception):
                ...
                raise GrantValidationError(f"Grant validation failed: {combined_message}", {"errors": errors, "data": data})
...
def check_grant_access(
    ...
    try:
        ...
        return {
            "allowed": False,
            "reason": f"Grant access check failed: {str(e)}"
        }
