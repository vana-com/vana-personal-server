...
def retrieve_grant_file(grant_url: str, timeout: int = 10) -> GrantFile:
    ...
    try:
        ...
        ipfs_hash = grant_url.replace("ipfs://", "") if grant_url.startswith("ipfs://") else grant_url
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if validate_grant_file_structure(grant_data):
                            ...
                            logger.warning(f"Invalid grant file structure from {gateway_url}")
                            continue
        ...
        raise NetworkError(f"Failed to retrieve grant file from any IPFS gateway: {grant_url}")
