...
@router.get("/artifacts/{operation_id}")
async def list_artifacts(
    ...
    Args:
        operation_id: The operation to list artifacts for
        authorization: Bearer token or signature for authentication
    """
    try:
        logger.info(f"[ARTIFACTS] List request: {operation_id}")
        
        # TODO: Implement proper grantee authentication
...
@router.get("/artifacts/{operation_id}/{artifact_path:path}")
async def download_artifact_simple(
    operation_id: str,
    artifact_path: str,
    request: Request
):
    """
    Simple artifact download for testing (no auth required).
    Only works in mock mode.
    """
    if not mock_mode_auth.is_mock_mode():
        raise HTTPException(
            status_code=403, 
            detail="Simple downloads only available in mock mode. Use POST /artifacts/download with signature."
        )
    
    try:
        logger.info(f"[ARTIFACTS] Simple download (mock mode): {operation_id}/{artifact_path}")
        ...
        logger.error(f"[ARTIFACTS] Error downloading {operation_id}/{artifact_path}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")